  ...
  >
    <template #value="slotProps">
        <div v-if="selectedCity">
          ...
          <span> - {{ selectedCity.state_code }}</span>
        </div>
      <div v-else>
    ...
    <template #option="slotProps">
        <div class="flex items-center">
          ...
          <span class="mr-2"> - {{ slotProps.option.state_code }}</span>
        </div>
    </template>
</Select>
</template>
...
const preSelectedCity = computed(() => store.getZipCodeData);
...
watch(() => selectedCity.value, (state) => {
...
async function fetchCities(state: string) {
  if (state) {
    await store.fetchCities(state).then(() => {
      if (preSelectedCity.value && preSelectedCity.value.city)
        setPreselectedCity(preSelectedCity.value.city)
    })
  }
}

async function setPreselectedCity(cityName: string) {
  if (cities.value && cities.value.length > 0) {
    selectedCity.value = cities.value.find((city: object) => city.name === cityName)
  }
}
